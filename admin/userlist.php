    ...
    <div class="container-fluid pt-5">
        <div class="container">
            <div class="text-center pb-2">
                <p class="section-title px-5"><span class="px-2">User List</span></p>
                <h1 class="mb-4">Users list</h1>
                <p class="text-primary text-center" style="font-size:13px;"><?= $aObject->userMethod(); ?></p>
            </div>
                ...
                <div class="col-lg-12 col-sm-12">
                    <div class="table-responsive">
                        <table class="table table-borederd">
                            ...
                            <?php
                                $dataQ = $aObject->getUserList();
                                $rowQ= $aObject->getUserList();
                                if ($rowQ > 0) {
                                    $x = 0;
                                        ...
                                        <?php
                                          if ($value['status'] == 1) { ?>
                                             <form action="" method="post">
                                                ...
                                                <input type="submit" name="Active" value="Active" class="btn btn-success" style="border-radius:0; height:28px; font-size: 10px;">
                                            </form>
                                       <?php  }else{  ?> 
                                            <form action="" method="post">
                                            <input type="hidden" name="uid" value="<?= $value['user_id']; ?>">
                                            <input type="submit" name="DeAtive" value="De-Active" class="btn btn-danger" style="border-radius:0; height:28px; font-size: 10px;">
                                          </form>
                                       <?php } ?>
                                         </td>
                                        <!-- <td><form action="" method="post">
                                            <input type="hidden" name="uid" value="<?php// $value['user_id']; ?>">
                                            <input type="submit" name="Remove" value="Remove" class="btn btn-danger" style="border-radius:0; height:28px; font-size: 10px;">
                                          </form>
                                              
                                          </td> -->
                                          <td>
                                              <a href="update.php?u=<?=$value['user_id']; ?>&e=<?=$value['email'] ?>&n=<?=$value['fullname']; ?>&r=<?=$value['role']; ?>" type="submit" name="Remove" value="Remove" class="btn btn-primary" style="border-radius:0; height:28px; font-size: 10px;">Update</a>
                                          </td>
                                    </tr>
